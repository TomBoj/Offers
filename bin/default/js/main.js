...
function append_offer_data(data) {
	...
	data.forEach(function(object) {
		...
		cancelButton.id = "cancel-" + object.offerId;
		cancelButton.value = object.offerId;
	...
	let cancelButtons = table.getElementsByTagName("button");
	for (var i = 0; i < cancelButtons.length; i++) {
		let cancelButton = cancelButtons[i];
		cancelButton.onclick = function() { 
			cancel_offer(cancelButton.value)
		};
	}
}
...
function cancel_offer(offerId) {
	let json_url = 'http://localhost:8081/offers/' + offerId + '/cancel';
	xmlhttp = new XMLHttpRequest();
